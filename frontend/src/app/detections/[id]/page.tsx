...
export default function DetectionDetailPage() {
  ...
  const { data, isLoading, error } = useQuery<DetectionDetail>({
    ...
    queryFn: () =>
      apiGet<DetectionDetail>(`/detections/${id}`, accessToken || undefined),
      ...
      <section className="rounded-lg border bg-white p-4">
        ...
        <div className="text-sm text-neutral-600">
          ...
          <span className="font-medium">
            {data.rule_id || data.title || "Unknown"}
          </span>
          {" · "}
        ...
        {primaryIp && (
          <p className="mt-1 text-sm text-neutral-600">
            Primary source IP (derived):{" "}
            <span className="font-medium">{primaryIp}</span>
          </p>
        )}
      </section>
